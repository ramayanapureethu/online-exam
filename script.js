/* NeonExam — script.js
   All features implemented: create exam, add questions (MCQ/TF), start exam, timer, navigation,
   mark for review, palette, auto-submit, score calc using array methods, review answers.
*/
...
(() => {
  ...
  // Data
  let exam = null; // {title,duration,passing,subject,questions:[]}
  let userAnswers = []; // parallel
  let marked = [];
  let currentIndex = 0;
  ...
  function submitExam(){
    clearInterval(timerInterval);
    // Results computed with array methods
